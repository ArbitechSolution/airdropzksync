...
function Airdrop({ currentAccount, web3Api, chainId }) {
  ...
  const getMerkleProof = async (address) => {
    try {
      ...
      const response = await axios.get(
        `https://airdrop.pepesupergrow.pro/merkle-proof/${address}`
      );
  ...
  const totalClaimed = async () => {
    ...
    try {
      ...
      console.log("totalClaimed", totalClaimedValue);
      setTotalClaimed(totalClaimedValue); // Set the totalClaimed value in state
    } catch (error) {
